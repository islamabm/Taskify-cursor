...
import { AddEmployeeStatusModal } from "../modals/AddEmployeeStatusModal";
import { EditEmployeeStatusModal } from "../modals/EditEmployeeStatusModal";


interface EmployeeStatusesTableProps<T> {
    readonly employeeStatuses: UserStatus[];
...
export function EmployeeStatusesTable<T>({ employeeStatuses, columnVisibilityName, placeHolder, filterColumnName, tableTitle, columnNames, inputTableName, excelConfig }: EmployeeStatusesTableProps<T>) {
    ...
    const TableEmployeeStatusesColumns: ColumnDef<UserStatus>[] = [

        {
            ...
            cell: ({ row }) => <div className="">{row.getValue("userStatus")}</div>,
        },
...

        {
            ...
            cell: (info) => (
                <div className="flex items-center  gap-2">
                    <EditEmployeeStatusModal taskStatusData={info.row.original} />
                    <DeleteValidationModal tooltipTitle='Delete status' alertDialogTitle='Are you sure you want to delete this status?' whichCmp='employeeStatus' usefulVariable={info.row.original.userStatusID} queryOptionalVariable='employeeId' />
                </div>)
        },
    ...
    const table = useReactTable({
        ...
        columns: TableEmployeeStatusesColumns,
