...
interface TaskTypesTableProps<T> {
...
export function TaskTypesTable<T>({ taskTypes, columnVisibilityName, placeHolder, filterColumnName, tableTitle, columnNames, inputTableName, excelConfig }: TaskTypesTableProps<T>) {
    ...
    const TableTaskTypesColumns: ColumnDef<TaskType>[] = [
...

        {
            ...
            cell: ({ row }) => <div className="">{row.getValue("ticketType")}</div>,
        },
        {
            ...
            cell: ({ row }) => <div className="">{row.original?.department?.departmentName}</div>,
        },
...

        {
            ...
            cell: (info) => (
                <div className="flex items-center gap-2">
                    ...
                    <DeleteValidationModal tooltipTitle='Delete task type' alertDialogTitle='Are you sure you want to delete this task type?' whichCmp='task-type' usefulVariable={+info.row.original.ticketTypeID} queryOptionalVariable='employeeId' />
                </div>)
        },
